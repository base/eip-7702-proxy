...
contract SetImplementationTest is EIP7702ProxyBase {
    ...
    function test_nonceIncrements_afterSuccessfulSetImplementation(uint8 numResets) public {
        ...
        for (uint8 i = 0; i < numResets; i++) {
            ...
            bytes memory signature = _signSetImplementationData(
                _EOA_PRIVATE_KEY, address(nextImplementation), "", address(nextImplementationValidator)
            );
            EIP7702Proxy(_eoa).setImplementation(
                address(nextImplementation), "", address(nextImplementationValidator), signature
            );
    ...
    function test_reverts_whenCalldataReverts() public {
        ...
        vm.expectRevert();
        EIP7702Proxy(_eoa).setImplementation(address(_implementation), reinitArgs, address(_validator), signature);
    }
    ...
    function test_reverts_whenValidatorReverts() public {
        ...
        EIP7702Proxy(_eoa).setImplementation(
            address(_implementation), reinitArgs, address(revertingValidator), signature
        );
    }
    ...
    function test_reverts_whenSignatureEmpty() public {
        ...
        EIP7702Proxy(_eoa).setImplementation(address(_implementation), "", address(_validator), signature);
    }
    ...
    function test_reverts_whenSignatureReplayedWithDifferentProxy(uint128 secondProxyPk) public {
        ...
        EIP7702Proxy(secondProxy).setImplementation(
            address(_implementation), initArgs, address(_validator), initSecondProxySignature
        );

        // create signature for original proxy
        bytes memory signature =
            _signSetImplementationData(_EOA_PRIVATE_KEY, address(_newImplementation), "", address(_validator));

        // attempt to play signature on second proxy
        vm.expectRevert(EIP7702Proxy.InvalidSignature.selector);
        EIP7702Proxy(secondProxy).setImplementation(address(_newImplementation), "", address(_validator), signature);
    }
    ...
    function test_reverts_whenSignatureReplayedWithDifferentArgs(bytes memory differentInitArgs) public {
        ...
        vm.expectRevert(EIP7702Proxy.InvalidSignature.selector);
        EIP7702Proxy(_eoa).setImplementation(
            address(_implementation), differentInitArgs, address(_validator), signature
        );
    }
    ...
    function test_reverts_whenSignatureReplayedWithDifferentValidator(address differentValidator) public {
        ...
        vm.expectRevert(EIP7702Proxy.InvalidSignature.selector);
        EIP7702Proxy(_eoa).setImplementation(address(_implementation), initArgs, differentValidator, signature);
    }
    ...
    function test_reverts_whenImplementationDoesNotMatchValidator() public {
        ...
        bytes memory signature =
            _signSetImplementationData(_EOA_PRIVATE_KEY, address(actualImpl), "", address(validator));
        ...
        EIP7702Proxy(_eoa).setImplementation(address(actualImpl), "", address(validator), signature);
    }
    ...
    function test_succeeds_whenImplementationMatchesValidator() public {
        ...
        bytes memory initArgs = _createInitArgs(_newOwner);
        bytes memory signature =
            _signSetImplementationData(_EOA_PRIVATE_KEY, address(_implementation), initArgs, address(validator));
    ...
    function test_reverts_whenValidatorReturnsWrongMagicValue() public {
        ...
        bytes memory initArgs = _createInitArgs(_newOwner);
        bytes memory signature =
            _signSetImplementationData(_EOA_PRIVATE_KEY, address(_implementation), initArgs, address(invalidValidator));

        vm.expectRevert(EIP7702Proxy.InvalidValidation.selector);
        EIP7702Proxy(_eoa).setImplementation(address(_implementation), initArgs, address(invalidValidator), signature);
    }
    ...
    function test_reverts_whenValidatorIsEOA() public {
        ...
        bytes memory initArgs = _createInitArgs(_newOwner);
        bytes memory signature =
            _signSetImplementationData(_EOA_PRIVATE_KEY, address(_implementation), initArgs, eoaValidator);

        vm.expectRevert();
        EIP7702Proxy(_eoa).setImplementation(address(_implementation), initArgs, eoaValidator, signature);
    }
    ...
    function test_reverts_whenValidatorIsNonCompliantContract() public {
        ...
        bytes memory initArgs = _createInitArgs(_newOwner);
        bytes memory signature = _signSetImplementationData(
            _EOA_PRIVATE_KEY, address(_implementation), initArgs, address(nonCompliantValidator)
        );

        vm.expectRevert();
        EIP7702Proxy(_eoa).setImplementation(
            address(_implementation), initArgs, address(nonCompliantValidator), signature
        );
    }
}
